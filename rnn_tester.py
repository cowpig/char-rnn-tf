...
def build_graph(hyperparams, n_steps, batch_size):
    ...
    y_ins = tf.split(1, y_in.get_shape()[1], y_in)
    ...
    for t in xrange(n_steps):
        ...
        tf_cost = tf.nn.softmax_cross_entropy_with_logits(
                no_softmax_vec, tf.squeeze(y_ins[t], [0]), 
                name="SequenceLoss/CrossEntropy{0}".format(i))
        tf_costvec.append(tf_cost)

    y_out = tf.concat(1, y_arr)
    states_out = tf.concat(2, states)
    cost = cross_entropy(y_out, y_in)

    tf_cost = tf.add_n(tf_costvec)

    return (x_in, y_in, states_in, states_out, y_out, cost, tf_cost)
...
if __name__ == '__main__':
    ...
    x_in, y_in, states_in, states_out, y_out, costs, tfc = build_graph(params, n_steps, batch_size)
    ...
    with tf.Session() as sesh:
        ...
        i=0
        while i < 5000:
            state, out, cost, tfcost, _ = sesh.run([states_out, y_out, costs, tfc, train], 
                            feed_dict={x_in:x, y_in:y, states_in:state})

            # print tf.Graph().get_operations()

            if i % 100 == 0:
                print "cost at epoch {}: {}, {}".format(i, cost, tfcost)
